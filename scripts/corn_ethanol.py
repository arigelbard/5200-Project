...
function drawLeft() {{
  ...
  svg.append('rect')
     ...
     .on('mousemove', function(event) {{
       ...
       // mx is relative to the overlay rect which starts at margin.left,
       // so subtract margin.left to get coordinates in the same space as xScale
       const mxInner = mx - margin.left;
       // Find nearest quarter by x position
       let best = quarters[0], bestDist = Infinity;
       quarters.forEach(q => {{
         const dist = Math.abs(xScale(q) - mxInner);
